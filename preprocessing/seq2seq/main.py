...
def main():
    ...
    #                 tweets = [Tweet(line.split(), line.split(), '1', '1') for i in range(2)] 
    #                 test_data, test_vocab, mappings = create_data(tweets, opt=opt, vocab=vocab,mappings=mappings)
    #                 prediction = test_evaluator.eval(test_data)
    #                 out_file.write(' '.join(prediction) + "\n")
    if opt.interactive:
        with open("input.txt", mode='r') as in_file:
             with open("output.txt", mode='w') as out_file:
                i = 0
                ...
                for line in tqdm(lines):
                    if(i<10000):
                        tweets.append(Tweet(line.split(), [], i, j, inputidx=j, outputidx=j))
                        j += 1
                        i = i + 1
                    else:
                        tweets.append(Tweet(line.split(), [], i, j, inputidx=j, outputidx=j))
                        ...
                        j += 1
                        test_data, test_vocab, mappings = create_data(tweets, opt=opt, vocab=vocab,mappings=mappings)
                        predictions = test_evaluator.eval(test_data)
                        prediction_strings = []
                        for prediction in predictions:
                            prediction_strings.append(' '.join(prediction) + "\n")
                        out_file.writelines(prediction_strings)
                        i = 0 
                        tweets = []
        ...
        logger.info("=======Eval on test set=============")
        pred_file = os.path.join(opt.save_dir, opt.outsave)
